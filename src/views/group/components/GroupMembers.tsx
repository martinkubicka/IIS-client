...
export const GroupMembers: React.FC<GroupMembersProps> = ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (
        ...
        const joinReqs =
          (await memberService.getJoinRequests(groupData?.handle)) ?? [];
        setJoinRequests(joinReqs);

        const moderatorReqs =
          (await memberService.getModeratorRequests(groupData?.handle)) ?? [];
      ...
      if (isVisibleMembers) {
        try {
          const memberDataResponse = await memberService.getGroupMembers(
            groupData?.handle,
            currentPage,
            itemsPerPage
          );
          ...
          const membersCount = await memberService.getGroupMembersCount(
            groupData?.handle
          );
          setTotalPages(
            Math.floor(membersCount / itemsPerPage) +
              (membersCount % itemsPerPage == 0 ? 0 : +1)
          );
        } catch (error) {}
      }
    };
  ...
  const onDelete = async () => {
    ...
    const membersResponse = await memberService.getGroupMembers(
      groupData?.handle,
      currentPage,
      itemsPerPage
    );
