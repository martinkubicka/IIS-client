...
export const Member: React.FC<MemberProps> = ({ member, onDelete, handle }) => {
  ...
  useEffect(() => {
    const getPermissions = async () => {
      ...
      if (
        loginService.getCookie("userRole") !== "" &&
  ...
  const saveSettings = async () => {
    try {
      ...
      enqueueSnackbar(
        error?.response?.data ?? "Error occured while updating member role.",
        {
          variant: "error",
          anchorOrigin: {
            vertical: "bottom",
        ...
        {isVisible ? (
          <div>
            <FormControl sx={{ width: 190 }}>
              ...
              <Select defaultValue={selectedRole?.toString()}>
                <Option value="0" onClick={() => handleRoleChange(0)}>
                  Admin
                </Option>
                <Option value="1" onClick={() => handleRoleChange(1)}>
                  Member
                </Option>
                <Option value="2" onClick={() => handleRoleChange(2)}>
                  Moderator
                </Option>
              </Select>
            </FormControl>
            ...
            <CardActions buttonFlex="1">
              <Box sx={{ display: "flex", gap: 1 }}>
                <Button disabled={!valueChanged} onClick={saveSettings}>
                  Save
                </Button>
